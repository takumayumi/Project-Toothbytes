...
public class TreatmentWindow extends JFrame {
    ...
    public TreatmentWindow(Patient patient) {
        action = 0;
        ...
        Action saveAction = new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                File pDir = new File("src\\toothbytes\\saves\\"+patient.getId());
                if(!pDir.exists()) {
                    pDir.mkdir();
                }
                 chart.paint(graphics2D);
                 try{
                     File a = new File("src\\toothbytes\\saves\\"+patient.getId()+"\\latest.png");
                     if(a.exists()){
                         System.out.println("deleted!");
                         a.delete();
                     }
                     ImageIO.write(image,"png", new File("src\\toothbytes\\saves\\"+patient.getId()+"\\latest.png"));
                     ImageIO.write(image,"png", new File("src\\toothbytes\\saves\\"+patient.getId()+"\\"+month + "-" + day + "-" + year+".png"));
                     }
                 catch(Exception ex){
                      ex.printStackTrace();
                     }
